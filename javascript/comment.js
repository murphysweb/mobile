...
$(function() {
  ...
  if (goods_draft) {
    ...
    if (pic != '') {
      ...
      for (var i = 0; i < pic.length; i++) {
        // addLi.before($('<li class="img"><div class="img_con"><img width="160px" height="160px" src = ' + pic[i] + '@160h_160w_1e_1c_2o' + ' data-show-src=' + pic[i] + '><span class="closeBtn"></span></div></li>'));
        addLi.before($('<li class="img"><div class="img_con"><img width="100%" height="100%" src = ' + pic[i] + ' data-show-src=' + pic[i] + '><span class="closeBtn"></span></div></li>'));
      }
      if (pic.length >= 9) {
        addLi.addClass('hide');
      }
  ...
  function picHandle() {
    ...
    function addPic() {
      ...
      if (title.innerHTML === '退货退款') {
        maxLength = '3';
      }
      let maxImageNumber = (maxLength - $('.add_comment_con .img').length).toString()
      //判断app版本
      let appV = '';
      if (isApp()) {
        appV = ua.getDvdAppVersion();
      }
      //todo iosapp580 581图片上传有问题 暂时写死限制上传1张 582版本解决
      let isIOS = ua.isIos();
      if((appV === '5.8.0' || appV === '5.8.1') && isIOS === true){
        maxImageNumber = "1";
      }
      if ((isApp() === true) && (ua.compareVersion('5.6.0', appV) <= 0)) {
        var div = $('<div class=\'add_pic_input\'></div>');
        addBtn.parent('li').append(div);
        addBtn.remove();
        div.on('click', function() {
          native.Browser.uploadImage({
            sourceType: '2',
            // maxImageNumber: (maxLength - $('.add_comment_con .img').length).toString(),
            ...
            success: function(data) {
              if (data && data.dataList) {
                ...
                for (var i = 0; i < dataList.length; i++) {
                  var data = fixImgUrl(dataList[i])
                  dealImgList(data);
                }
                //上传到最大值后隐藏上传input
                let img = $('li.img');
                if (img.length === Number(maxLength)) {
                  let add = $('li.add')[0];
