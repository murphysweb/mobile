...
function httpsByHttp(){
  var hostp = Units.getQuery('hostp')
  var aList = $('a')
  for (var i=0;i<aList.length;i++){
    if (aList[i] && aList[i].href && aList[i].href.substring(0,5)=='http:' && new RegExp('davdian','g').test(aList[i].href)){
      $('a')[i].href = 'https:' + aList[i].href.substring(5,aList[i].href.length)
    }
  }
}
httpsByHttp()
