...
jQuery(document).ready(function ($) {
  //物流详情——>物流投诉跳转
  // $('#styleLink').on('click', function () {
  //   var params = location.href.split("?")[1];
  //   window.location.href = '/orderComplain.html?'+params;
  // });
  $(document).on('click','.orderServer',function(event){
    var params = location.href.split("?")[1];
    window.location.href = '/orderComplain.html?'+params + '&logisticsCode=' + $(this).parents('.order_goods_group_wl_con').attr('data-logistic-code')
  })

  //物流投诉ajax请求
  getOrederAjax();

  function getOrederAjax() {
    var deliveryId = location.href.split("?")[1];
    var did = (deliveryId.slice(4,deliveryId.length));

    $.ajax({
      url: "/api/mg/order/workOrder/schedule?_=" + Date.now(),
      dataType: "JSON",
      type: "POST",
      data: strSign({
        deliveryId:did
        ...
        if (res.code == 0) {
          ...
          dataMessage.forEach(function (item) {
            var str = "<div class='pl'>" +
              "<span class='first'>" + item.issusType + "</span>" +
              "<span class='last'>" + fmtDate(item.closeTime * 1000) + "</span>" +
              "<p>" + item.message + "</p>" +
              "</div>";
            $(".orderMessageBox").append(str);
          })
        }else {
