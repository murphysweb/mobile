...
$(function () {
  ...
  function loadMore(){
    ...
    $(".goods-list-template").each(function () {
      var $el = $(this),flag = false;
      var goods = $el.attr("data-goods-id");
      if(goods&&goods.length&&!flag&&!$el.data('ajaxingTemplateData')){
        flag = true;
        var goodsList = goods.split(",");
        if(goodsList.length>12){
          let otherList = goodsList.splice(12);
          $el.attr("data-goods-id",otherList.join(","))
          goods = goodsList.join(",");
        }else{
           goods = goodsList.join(",");
           $el.removeAttr("data-goods-id");
        }

        renderTemplate($el,goods,goodsList.length<=12);
      }
    });
  }
  function showHeader() {
  ...
  window.aa = function (a){window.open(a)}
  // 新逻辑 根据id增加商品
  $(".goods-list-template").each(function () {
    var $el = $(this);
    var goods = $el.attr("data-goods-id");
    ...
    if(goodsList.length>12){
      ...
      $el.attr("data-goods-id",otherList.join(","))
      goods = goodsList.join(",");
      $el.html('<div class="more" style="text-align:center">数据加载中</div>');
    }else{
      $el.removeAttr("data-goods-id");
    }
  ...
  function renderTemplate($el,goods,last){
    if($el.data('ajaxingTemplateData')){
      return false
    }
    $el.data('ajaxingTemplateData',true)
    $.ajax({
      ...
      dataType: "json",
      error:function(){
        $el.data('ajaxingTemplateData',false)
      },
      success: function (res) {
        $el.data('ajaxingTemplateData',false)
        var data = res.data;
        var str
        if($el.find(".goods_group").length){
          str = "";
        }else{
           str = '<div class="goods_group">';
        }
        for (var i = 0; i < data.length; i++) {
          ...
          if (window.Units.isIOS()){
            // 老板专题使用command跳转规范
            if(d.command && d.command.content){
              str += '<a href="' + d.command.content + '">';
            }else{
              str += '<a href="/' + d.goods_id + '.html?rl=' + res.referer.rl + '&rp=' + res.referer.rp + '">';
            }
          } else {
            if(d.command && d.command.content){
              str += '<a onclick="aa('+'\''+d.command.content+ '\')">';
            }else{
              str += '<a onclick="aa('+'\'/'+ d.goods_id + '.html?rl=' + res.referer.rl + '&rp=' + res.referer.rp + '\'' +')">';
            }
          }
          str += '<div class="goods_img">' +
            (i < 40 ? ('<img src=\"' + d.goods_img + '\">') : ('<img src="//9i.dvmama.com/free/loading_320_320_v2.png" data-original=\"' + d.goods_img + '\">')) +
        ...
        if($el.find(".goods_group").length){
          ...
          if(last){
            str += '<div class="more"  style="text-align:center;clear:both"></div>'
          }else{
            str += '<div class="more"  style="text-align:center;clear:both">数据加载中</div>'
          }
        }

        str +='</div>';
        if($el.find(".goods_group").length){
          $el.find(".goods_group").find(".more").before($(str));
          if(last){
            $el.find(".goods_group").find(".more").html("")
          }
        }else{
          $el.html(str);
        }
...
(function(){
  if(window.descContent){
    window.descContent = window.share_seller_name+' 推荐：'+ window.descContent;
  }else{
    window.descContent = window.share_seller_name+' 推荐';
  }
})()
var scriptjs = require('scriptjs');
// 头部标题栏
(function () {
  if(ua.isDvdApp() && ua.compareVersion(ua.getDvdAppVersion(), '5.2.0') < 0) return;
  //微信适配--mahenan-20180627
  if(ua.isWeiXin()){
    var top_container=document.querySelector(".top_container");
    if(top_container){
      top_container.parentNode.removeChild(top_container);
    }
    // $("body").css("paddingTop",$(".top_other").height() + "px!important;");
    $("body").get(0).setAttribute("style","padding-top: "+ $(".top_other").height() +"px!important");
    let getEl = function () {
    ...
    scriptjs('//3n.dvmama.com/vue/2.4.2/vue.min.js', function() {
      new Vue({
        el: getEl(),
        components: {
          'dvd-service-com-go-home': require('dvd-service-com-go-home'),
        },
        template: '<dvd-service-com-go-home></dvd-service-com-go-home>',
        mounted(){
        this.$nextTick(function(){
          $(".dvd-service-com-go-home img").css("bottom","0.8rem");
        })
        }
      });
    });
  ...
  scriptjs('//3n.dvmama.com/vue/2.4.2/vue.min.js', function(){
    ...
    // var title = document.querySelector('.look_title') && document.querySelector('.look_title').innerHTML || document.title;
    // title = title.replace(/"/ig, '');
    var el = '.top0';
    if(!document.querySelector('.top0')){
      var top_title_520 = document.createElement('div');
      top_title_520.className = 'top_title_520';
      el = 'top_title_520';
    }
    new Vue({
      el: el,
      ...
      template: `<com-top-title title="${document.title}" home slide share>${top_other && top_other.outerHTML || ''}</com-top-title>`,
    });
    if(top_other){
      top_other.parentNode.removeChild(top_other);
    }
    // 去掉多余
    var top0 = document.querySelector('.top0');
    if(top0){
      top0.parentNode.removeChild(top0);
    }
    // 去掉app逻辑
    setTimeout(function(){
      document.body.className = document.body.className.replace('in_app', '');
    }, 1);
  });
})();
