  ...
  <div v-if='allkind' class="order_list_item dav-shadow" v-for="item in list_type" data-for-id="{{ item.id }}" data-for='11' data-for-delivery-id="{{item.delivery_id}}">
    <div class="order_name">
      <a href="{{ item.shop_url }}">
        ...
        <span class="shop_title">{{ item.shop_name }}</span>
    ...
    <div class="sum">
      <span v-if="item.is_presale_order && (item.type == '3' || item.type == '4') ">
        ...
        <span v-if="item.presale_info.type == 'reserve'">
          <span>应付定金：</span>
          <span class="payment">
            <span class="fz_12">￥</span>{{item.presale_info.reserve_info.payment}}</span>
        </span>
        <span v-if="item.presale_info.type == 'final'">
          <span>应付尾款：</span>
          <span class="payment">
            <span class="fz_12">￥</span>{{item.presale_info.final_info.payment}}</span>
        </span>
      </span>
      <span v-else>
        <span v-else>金额：</span>
    ...
    <div v-if="item | isFinalBtn" class="order_buttons order_list_buttons clearfix">
      ...
      <!-- 支付尾款，显示按钮 -->
      <a v-show="item | orderFinalBtn" v-if='item.order_type !=1 && item.order_type !=2' class="dav-btn btn-white order-btn-red pull-right  order-buy-once-more" href="javascript:void(0)" @click="pay_weikuan(item.pay_url)">支付尾款</a>
      <!--该显示哪些信息-->
      ...
      <a v-show="item | checkship" class="dav-btn btn-white pull-right order-check-wl" href="/m/logistics_information.html?oid={{item.id}}&did={{item.delivery_id}}">查看物流</a>
      <a v-show="item | shipping" class="dav-btn btn-white pull-right order-check-wl" href="/m/logistics_information.html?oid={{item.id}}&did={{item.delivery_id}}">查看物流</a>
    ...
    <!-- 支付尾款，显示日期 -->
    <div v-show="item | orderFinal" class="order_presale">
      <span v-if="item.presale_info && item.presale_info.final_info">{{ item.presale_info.final_info.paytime_start | changeDate }}</span> 开始支付尾款</div>
  ...
  <div class="order_list_item type_4 dav-shadow" v-if='other'>
    <ul class="be_evaluated_list">
      <li v-for='item in list_type' data-goods_id="{{item.goods_id}}">
        ...
        <div v-if="item.command && item.command.content">
          <a :href="item.command.content" class="goods_pic">
            <img :src='item.goods_img'>
          </a>
          <div class="goods_con">
            <a :href="item.command.content" class="goods_title">{{ item.goods_name }}</a>
    ...
    <div class="modalCont" v-if="reasonData">
      <label :data-id="item.reasonId" v-for="item of reasonData.data.reasonList" class='navLabel' :for="item.reasonId">
        <div @click="handleReason" :data-name="item.reasonName" :data-id="item.reasonId" class='navList'>
          <div :data-name="item.reasonName" :data-id="item.reasonId" class="flex1">{{ item.reasonName }}</div>
          <div>
...
export default {
  ...
  data: function() {
    return {
      ...
      currentType:
        +location.href.substr(location.href.indexOf('type=') + 5, 1) || 0,
      preLoadFlag: [],
  ...
  created: function() {
    ...
    if (!isPrivateMode) {
      ...
      if (patharr.length > 2) {
        ...
        if (
          lastPath == 'order_detail' ||
          lastPath == 'order_delivery' ||
          lastPath == 'detail' ||
          lastPath == 'grade' ||
          lastPath == 'add_address' ||
          lastPath == 'agent_pay' ||
          lastPath == "cart_confirm" &&
            $.cookie &&
            !$.cookie("dvd_cart_to_confirm")
        ) {
      ...
      if (this.isConfirm) {
        ...
        $.ajax({
          ...
          success: function(result) {
            ...
            var bg = $('.cancle_' + scope.dataId + '').parents(
              '.order_buttons'
            );
            var og = $('.cancle_' + scope.dataId + '').parents(
              '.order_list_item'
            );
            bg.find('a').remove();
            bg.append(
              $(
                '<a class="dav-btn pull-right btn-white btn-disable">' +
                  info +
                  '</a>'
              )
            ); // 删掉取消订单 增加取消的结果
              ...
              if (!result['error']) {
                ...
                if (result['data'].length) {
                  ...
                  result.data.map(function(value, index) {
                    if (
                      value.is_presale_order &&
                      value.presale_info.type == 'reserve'
                    ) {
                    ...
                    if (
                      ...
                      value.type != 2 &&
                      value.is_presale_order &&
                      value.presale_info.type == 'final'
                    ) {
                      if (
                        Date.now() >
                        value.presale_info.final_info.paytime_end * 1000
                      ) {
                        value.type = 4;
                      }
                    }
                  ...
                  scope.allData[orderType] = scope.allData[orderType].concat(
                    result.data
                  ); // 缓存中添加数据
  ...
  filters: {
    ...
    isFinalBtn(value) {
      if (value.is_presale_order && value.presale_info.type == 'final') {
        if (
          Date.now() <
          Number(value.presale_info.final_info.paytime_start) * 1000
        ) {
        } else {
          return true;
        }
    ...
    orderFinalBtn(value) {
      if (value.is_new_seller_order == false && value.type == 3) {
        if (value.is_presale_order && value.presale_info.type == 'final') {
          if (
            Date.now() <
              Number(value.presale_info.final_info.paytime_end) * 1000 &&
            Date.now() >
              Number(value.presale_info.final_info.paytime_start) * 1000
          ) {
            return true;
  ...
  watch: {list_type: 'handleCurrentPage'}
};
</script>
