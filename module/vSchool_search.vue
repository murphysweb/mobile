    ...
    <div v-if="ifshowhot" class="search_history">
      ...
      <ul class="history_list">
        <li v-for="item in hotList" :style="{color:color1(item.colorType)}"
            @click="searchs(item.name,item.command)"
            >{{item.name}}</li>
      </ul>
    </div>
...
//          scope.no_more = scope.vscholl_search_historytData.no_more === 'true';
    ...
    methods: {
      ...
      searchs: function (msg, command) {
        if (command.content == '') {
          this.addWord2List(msg);
          this.addList(msg,command.ref);
        } else {
          this.addWord2List(msg);
      ...
      refreshlist: function (v, page,ref) {
        var scope = this,
          data = {};
        data.keywords = v;
        data.page = page;
