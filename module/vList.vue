<template>
  <div class="classroom_container">
    ...
    <!--标题-->
    <com-top-title :title="isShow_name" slide :search="!ua.isDvdApp()&&!getQuery('no_search')" :share="ua.isDvdApp()&&!isMoney" :search-click="function(){window.location.href = '/class_category.html';}" :back-click="(inApp&&isMoney)?closePage:null">
      <v-list-switcher slot="bottom" style="position: absolute;bottom: -38px;left: 0;width: 100%;"
                       v-if="haveMenu && (!ua.isDvdApp() || ua.isDvdApp() && ua.compareVersion(ua.getDvdAppVersion(), '5.2.0')) >= 0" class="out_switch" :list="switcherListDate" :init-category="category"
                       v-on:category="changeCategory"></v-list-switcher>
    </com-top-title>
    ...
    <!--520以下app-->
    <v-list-switcher slot="bottom" style="position: fixed;top: 0;width: 100%;"
                     v-if="(haveMenu && ua.isDvdApp() && ua.compareVersion(ua.getDvdAppVersion(), '5.2.0') < 0)" class="out_switch" :list="switcherListDate" :init-category="category"
                     v-on:category="changeCategory"></v-list-switcher>
    <div :style="{'height':haveMenu && ua.isDvdApp() && ua.compareVersion(ua.getDvdAppVersion(), '5.2.0') < 0 ? '38px' : null}"></div>


    <!--<div class='content-top' v-if='!inApp && haveMenu'></div>-->
    <!--<div class='content-top1' v-if='!inApp && !haveMenu'></div>-->
    <!--<v-list-switcher v-if='inApp && haveMenu' class="out_switch out_switch_top" :list="switcherListDate"-->
    <!--:init-category="category" v-on:category="changeCategory"></v-list-switcher>-->
    <div class='content-top-top'  v-if="haveMenu && (!ua.isDvdApp() || ua.isDvdApp() && ua.compareVersion(ua.getDvdAppVersion(), '5.2.0') >= 0)"></div>
    <div class='content-top-top2'  v-if="!haveMenu && (!ua.isDvdApp() || ua.isDvdApp() && ua.compareVersion(ua.getDvdAppVersion(), '5.2.0') >= 0)"></div>

    <v-list-content :init-list="contentListData" :has-more="hasMore" :category="category" v-on:top="changeTop"
                    v-on:praise="praise"></v-list-content>
...
<script>
  ...
  import comFooter from '../src/component/com-footer/com-footer.vue'
  ...
  export default{
    data(){
      return {
        ...
        isMoney:false,
        haveMenu: true,
        menuName: getQuery("isShow") && getQuery("isShow").indexOf('-')>0 && getQuery("isShow").split('-').length>1 && '亲子时光' ||  document.title || '动态',
        needSaveParas: ['pageIndex', 'category', 'hasMore', 'contentListData', 'switcherListDate', 'bannerListData', 'pos'],
        isShow: getQuery("isShow"),
        window: window,
        ua: ua,
        getQuery:getQuery,
        isShow_name:getQuery("showName")?decodeURIComponent(getQuery("showName")):((getQuery("isShow") && getQuery("isShow").indexOf('-')>0 && getQuery("isShow").split('-').length>1 && '亲子时光') ||  document.title || '动态')
      }
    },
    watch: {},
    created(){
      ...
      this.isMoney = !!this.getQuery("isMoney")
    },
    updated(){
...
//              imgUrl: "http://9i.dvmama.com/free/2017/08/30/210_210_f115620a5e0d745863faaa11f7b49aa3.jpeg"
//            };
        ...
        if (window.Units.getQuery('menuId') || window.Units.getQuery('menuId') == 0) {
          ...
          if (sessionList && sessionList.length >= 2 && JSON.parse(sessionStorage.getItem("history"))[JSON.parse(sessionStorage.getItem("history")).length - 1].path == JSON.parse(sessionStorage.getItem("history"))[JSON.parse(sessionStorage.getItem("history")).length - 2].path) {
            ...
            for (var a in sessionStorage) {
              if (!(a == 'history' || a == 'v_list'||a == "earn_pos"))
      ...
      getMenuName(data){
        ...
        for (let i = 0; i < data.length; i++) {
          if (data[i].menuId == this.category) {
            if (that.isShowFn() && that.isShowFn().length>1){
              this.menuName = '亲子时光'
            } else {
            ...
            if(ua.isDvdApp() && ua.compareVersion(ua.getDvdAppVersion(), '5.2.0') < 0) {
              if (this.inApp && window.bravetime && bravetime.setHead) {
                window.bravetime && bravetime.setHead({title: that.menuName})
              }
            }
          }
        }
          ...
          if (result.code == '0') {
            ...
            if (result.data && result.data.menuData && result.data.menuData.menuList) {
              if (that.isShow) {
                ...
                if (that.isShowFn() && that.isShowFn().length>1){
                  that.isShow_name= '亲子时光';
                  document.title = '亲子时光';
                  if(getQuery("showName")){
                    document.title = that.isShow_name = decodeURIComponent(getQuery("showName"));
                  }
                } else {
                  that.isShow_name = that.switcherListDate[0].title;
                  document.title = that.isShow_name;
                }
              } else {
